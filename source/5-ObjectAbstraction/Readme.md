...
If we apply this to a computational object and its abstraction, it only makes sense in terms of its wider relations and a recognition of its conditions of operation, and only then if there is a point to expose these conditions such that they can be improved, not least with better abstractions — that are goal-oriented like a game. Computational objects allow for a different perspective on lived conditions and the ways we perceive the world. The worldviews on offer are far too often quite objectionable, and we only need to think of game-worlds to see overt problems of racial and gendered abstraction. Therein lies the motivation for this chapter, to make better object abstractions or at least ones with a sense of purpose. 
