...
“self-supervised representation learning” (as, for example, generative adversarial networks, GANs) – a method of training of the neural network that doesn’t rely on the tagging of input images by humans, or training by captcha or mechanical turk – to turn to how the machine generates groupings based on its own analysis. Bias becomes more complex in such a scenario, and “algorithmic governmentality” more nuanced – expressing what Stiegler would call the logic of “generalized automatization”.

Might such an approach open up alternatives to the conflation of computational logics and models of democracy that Komar + Melamid exposed, or a deeper way into this perhaps? In this model, the principle of grouping responds to similarities that the machine perceives – and rather than privileging these high confidence categories, they would then be discarded, and instead the remaining images, the ones that defy easy groupings (the low confidence images) – the “least wanted” would become the “most wanted” in other words.

Why do I think this important?
To what extent does the distorted democracy we currently experience find parallel in systems that generalize massive amounts of data in particular ways, and that reduce knowledge (if not truth) to commodity form?

//////

We might say that in beginning to think about the operations of machine learning in this way, we follow its logic in drawing upon the what we have learnt thus far and then speculate, or predict, future possibilities. We use this last chapter as a way to point to further work to be undertaken in this way and to reflect on machine learning as a set of methods that learn from data in parallel to our and your own experience of learning through practice.
...
[^agre]: Philip E. Agre, "Toward a Critical Technical Practice: Lessons Learned in Trying to Reform AI", in Geoff Bowker, Les Gasser, Leigh Star, and Bill Turner, eds, *Bridging the Great Divide: Social Science, Technical Systems, and Cooperative Work* (Erlbaum, 1997). Agre says: "A critical technical practice will, at least for the foreseeable future, require a split identity - one foot planted in the craft work of design and the other foot planted in the reflexive work of critique."[^agre2] The challenge is to work across and between these modes and learn from both.
