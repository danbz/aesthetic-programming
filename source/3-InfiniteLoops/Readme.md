...
As we have established loops execute a block of code as long as a specified condition is true. In this closing section to the chapter it seems appropriate to emphasise that while/for-loops allow code to be executed repeatedly based on a given condition like this. The loop can be thought of as a repeating *if* statement and so a good way to challenge conventional structures of linear time and how computers understand time differently from humans. Programming challenges many of our precepts of how we consider time to be organised, and how the now is rendered via different time-specific parameters and conditions that are implemented in our streaming and network infrastructure like with the activities behind a throbber.[^soon2] We hope it is already clear that machine-time operates at a different register, and what it means to begin and end a given process.
...
[^soon2]: Soon, “Throbber: Executing Micro-temporal Streams.” *Computational Culture*.
[^soon3]: Soon, “Throbber: Executing Micro-temporal Streams.” *Computational Culture*.
