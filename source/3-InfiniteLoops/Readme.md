...
Aside from seductive images of autocannibalism, loops are related to procedures of repetition, control and automation, and are mirrored in everyday repetitive procedures. This can be demonstrated in the following paradoxical word-play: "The next sentence is true. The previous is false".[^Hofstadter] In computer programming, a loop allows repeated execution of a fragment of source code that continues until a given condition is met, such as true or false. Indeed a loop becomes an infinite loop if a condition never becomes false. 

It was Augusta Ada Byron Lovelace who first introduced the concept of a programmatic loop in the early nineteenth century. She recognised that there were repeatable operations in the conceptual design of the first automatic, general-purpose computing machine ever designed,[^Kim] namely Charles Babbage's Analytical Engine. A loop which she called a 'cycle' appears in her "Note G" on the Analytical Engine, and this sets a precedent for the direction in which digital computers would be later developed. Her notes describe the program procedures, also called the Bernoulli numbers program, can be seen in the diagram below. It utilises two loops to indicate the repetition of a set of instructions with conditions,[^Kim2] minimizing duplicate efforts to write the repeatable operation repeatedly. 
...
The concept of a loop in modern coding practice is highly influenced by these initial insights into the handling of repeated machine operations expressed in a concrete diagramatic manner, which depicts the essence of repetition and condition in a cycle. Modern high-level programming language include a loop function, allowing a fragment of source code to be repeatedly executed. For instance, in `p5.js` the `draw()` function will run continuously until the program is stopped. We will continue with examples but suffice to say that loops are among the most basic and powerful of programming concepts. 

#### 3.1.1 Start()
The example we use centrally in this chapter is the graphical spinning wheel icon of a so-called 'throbber'[^throbber] that indicates when a computer programme is performing an action such as downloading content or performing intensive calculations. We find this to be an especially evocative symbol as it stands for the hidden processes and the complexity of multiple temporalities that are running during a given computational task. We use this to guide the programming tasks, but also because it acts as a cultural object for us to think through the idea of loops more conceptually. 

////////Users usually encounter this ubiquitous spinning icon during the loading and streaming of data content. It shows we are waiting for something to happen but what exactly is not clear at all. Something is in-progress, but we know little more as it does not indicate any completed or finished status or progress (as a progress bar does, for instance) and all that is presented is a spinning icon, perceived as repeatedly spinning under constant speed as well as hinting at invisible background activities for an indeterminate and unforeseeable timespan. The icon expresses various dimensions of time at our times. 
