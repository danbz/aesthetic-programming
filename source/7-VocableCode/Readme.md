...
[^master]: This master-slave is considered as "an opporessive metaphor" according to The Internet Engineering Task Force (IETF). It is suggested that language like master-slave is not just only oppressive and hurts people, but also inaccurate in both technical and historical way. Many communities have decided to stop using it, such as Django and Python, and have replaced with alternative terms such as "main" as in the case of Github. See, https://tools.ietf.org/id/draft-knodel-terminology-00.html#rfc.section.1.1. 
