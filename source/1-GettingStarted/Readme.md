...
For simplicy, we use the GitLab's web interface for some of our writing and teaching. Specifically for students to familiar with the writing of README file[^readme] and also aware that JavaScript(JS) can be run online via a web browser as RUNME.[^runme] We also use GitLab for peer feedback so that students can read other people work with both README and RUNME.
